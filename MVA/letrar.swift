...
class letrar: UIViewController {
    
    var letraminuscula = AVAudioPlayer()
    var rinoceronte = AVAudioPlayer()
    ...
    override func viewDidLoad() {
        ...
        let movieURL:NSURL? = NSURL(string: "http://colegiosoer.com.br/letrar.3gp")
    ...
    @IBAction func animal(sender: AnyObject) {
        ...
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrarrinoceronte", ofType: "wav")!)
        ...
        // Play the sound
        do {
            rinoceronte = try AVAudioPlayer(contentsOfURL: alertSound)
        } catch _{
        }
        
      
        rinoceronte.prepareToPlay()
        rinoceronte.play()
    ...
    @IBAction func letraminuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrarminuscula", ofType: "wav")!)
    ...
    @IBAction func letramaiuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrarmaiuscula", ofType: "wav")!)
