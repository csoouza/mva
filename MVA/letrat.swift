...
class letrat: UIViewController {
    
    var letraminuscula = AVAudioPlayer()
    var tartaruga = AVAudioPlayer()
    ...
    override func viewDidLoad() {
        ...
        let movieURL:NSURL? = NSURL(string: "http://colegiosoer.com.br/letrat.3gp")
    ...
    @IBAction func animal(sender: AnyObject) {
        ...
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrattartaruga", ofType: "wav")!)
        ...
        // Play the sound
        do {
            tartaruga = try AVAudioPlayer(contentsOfURL: alertSound)
        } catch _{
        }
        
      
        tartaruga.prepareToPlay()
        tartaruga.play()
    ...
    @IBAction func letraminuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letratminuscula", ofType: "wav")!)
    ...
    @IBAction func letramaiuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letratmaiuscula", ofType: "wav")!)
