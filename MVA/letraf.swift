...
class letraf: UIViewController {
    
    var letraminuscula = AVAudioPlayer()
    var elefante = AVAudioPlayer()
    ...
    override func viewDidLoad() {
        ...
        let movieURL:NSURL? = NSURL(string: "http://colegiosoer.com.br/letraf.3gp")
    ...
    @IBAction func animal(sender: AnyObject) {
        ...
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letraffoca", ofType: "wav")!)
        ...
        // Play the sound
        do {
            elefante = try AVAudioPlayer(contentsOfURL: alertSound)
        } catch _{
        }
        
      
        elefante.prepareToPlay()
        elefante.play()
    ...
    @IBAction func letraminuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrafminuscula", ofType: "wav")!)
    ...
    @IBAction func letramaiuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrafmaiuscula", ofType: "wav")!)
