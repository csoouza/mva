...
class letras: UIViewController {
    
    var letraminuscula = AVAudioPlayer()
    var sapo = AVAudioPlayer()
    ...
    override func viewDidLoad() {
        ...
        let movieURL:NSURL? = NSURL(string: "http://colegiosoer.com.br/letras.3gp")
    ...
    @IBAction func animal(sender: AnyObject) {
        ...
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrassapo", ofType: "wav")!)
        ...
        // Play the sound
        do {
            sapo = try AVAudioPlayer(contentsOfURL: alertSound)
        } catch _{
        }
    ...
    @IBAction func letraminuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrasminuscula", ofType: "wav")!)
    ...
    @IBAction func letramaiuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrasmaiuscula", ofType: "wav")!)
