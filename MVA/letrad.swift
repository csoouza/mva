...
class letrad: UIViewController {
    
    var letraminuscula = AVAudioPlayer()
    var colocaraquianimalcomd = AVAudioPlayer()
    ...
    override func viewDidLoad() {
        ...
        let movieURL:NSURL? = NSURL(string: "http://colegiosoer.com.br/letrad.3gp")
    ...
    @IBAction func animal(sender: AnyObject) {
        ...
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letraddromedario", ofType: "wav")!)
        ...
        // Play the sound
        do {
            colocaraquianimalcomd = try AVAudioPlayer(contentsOfURL: alertSound)
        } catch _{
        }
        
      
        colocaraquianimalcomd.prepareToPlay()
        colocaraquianimalcomd.play()
    ...
    @IBAction func letraminuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letradminuscula", ofType: "wav")!)
    ...
    @IBAction func letramaiuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letradmaiuscula", ofType: "wav")!)
