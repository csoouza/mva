...
class letraw: UIViewController {
    
    var letraminuscula = AVAudioPlayer()
    var wallaby = AVAudioPlayer()
    ...
    override func viewDidLoad() {
        ...
        let movieURL:NSURL? = NSURL(string: "http://colegiosoer.com.br/letraw.3gp")
    ...
    @IBAction func animal(sender: AnyObject) {
        ...
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrawwallaby", ofType: "wav")!)
        ...
        // Play the sound
        do {
            wallaby = try AVAudioPlayer(contentsOfURL: alertSound)
        } catch _{
        }
        
      
        wallaby.prepareToPlay()
        wallaby.play()
    ...
    @IBAction func letraminuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrawminuscula", ofType: "wav")!)
    ...
    @IBAction func letramaiuscula(sender: AnyObject) {
        // Set the sound file name & extension
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("letrawmaiuscula", ofType: "wav")!)
